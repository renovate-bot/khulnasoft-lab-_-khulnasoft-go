...
func NewResourceGroupForZone(zone Zone) ResourceGroup {
	return NewResourceGroup(fmt.Sprintf("com.khulnasoft.api.account.zone.%s", zone.ID))
}
...
func NewResourceGroupForAccount(account Account) ResourceGroup {
	return NewResourceGroup(fmt.Sprintf("com.khulnasoft.api.account.%s", account.ID))
}

// NewResourceGroup takes a Khulnasoft-formatted key (e.g. 'com.khulnasoft.api.%s') and
